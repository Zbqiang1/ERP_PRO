<template>
  <div class="purchase-return-list-container">
    <!-- 搜索表单 -->
    ...
    >
      ...
      <el-table :data="formData.details" border size="small">
        ...
        <el-table-column prop="returnQty" label="退货数量" width="110">
          <template #default="{ row }">
            <el-input-number v-model="row.returnQty" :min="0" :max="row.receivedQty" :precision="0" size="small" controls-position="right" style="width: 100%" />
          </template>
        </el-table-column>
...
import { purchaseReturn } from '@/api/modules/purchase'
...
const loadPOOptions = async () => {
  try {
    const res = await purchaseOrder.page({ page: 1, pageSize: 999, status: 'CONFIRMED,PARTIAL_RECEIVED,COMPLETED' })
...
const handlePOChange = async (poId) => {
  ...
  try {
    ...
    formData.details = (data.details || []).map(d => ({
      ...
      receivedQty: d.receivedQty || d.quantity || 0,
      unit: d.unit,
      returnQty: 0,
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await purchaseReturn.page(params)
...
const handleSubmit = async () => {
  ...
  if (validDetails.length === 0) {
    ElMessage.warning('请至少填写一条退货数量大于0的明细')
    return
  }
  submitLoading.value = true
  try {
    await purchaseReturn.add({
      ...formData,
      details: validDetails,
      totalAmount: computedReturnTotal.value
    })
...
const handleView = async (row) => {
  try {
    const res = await purchaseReturn.getById(row.id)
...
.purchase-return-list-container {
  padding: 16px;
